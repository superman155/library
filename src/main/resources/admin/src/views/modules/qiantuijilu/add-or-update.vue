...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="签退时间" prop="qiantuishijian" >
					<el-date-picker
						...
						:picker-options="qiantuishijianPickerOptions"
						placeholder="签退时间"
					></el-date-picker>
				</el-form-item>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="签退备注" prop="qiantuibeizhu" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.qiantuibeizhu" label="签退备注" prop="qiantuibeizhu"  class="textBox">
				<span class="text">{{ruleForm.qiantuibeizhu}}</span>
			</el-form-item>
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `qiantuijilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.qiantuijiluCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
