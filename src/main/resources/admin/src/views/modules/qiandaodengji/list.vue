...
<template>
	<div class="main-content" :style='{"padding":"50px 30px 30px"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"border":"1px solid #00000050","padding":"0","margin":"0 0 0px","borderRadius":"10px 10px 0 0","background":"#fff","borderWidth":"1px 1px 0","width":"100%"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","padding":"10px","margin":"0px 0 0px -4px","flexWrap":"wrap","justifyContent":"flex-end","display":"flex"}'>
					<el-button class="add" v-if="isAuth('qiandaodengji','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('qiandaodengji','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"1px solid #00000050","width":"100%","padding":"0","boxShadow":"0px 3px 6px #8558ff80","borderRadius":"0 0 10px 10px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column  :resizable='true' prop="touxiang" width="200" label="人脸识别">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('qiandaodengji','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<!-- [Ljava.lang.String;@62a6ce9b -->
							<!-- [Ljava.lang.String;@17acd2ac -->
							<el-button class="btn8" v-if="isAuth('qiandaodengji','签退')" @click="qiantuijiluCrossAddOrUpdateHandler(scope.row,'cross','是','','[1]','已签退')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								签退
							</el-button>
							<el-button class="edit" v-if=" isAuth('qiandaodengji','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('qiandaodengji','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<el-dialog
			...
			width="800">
			<div id="qiandaoshijianChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
...
<script>
	...
	import qiantuijiluCrossAddOrUpdate from "../qiantuijilu/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				qiantuijiluCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			qiantuijiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.qiantuijiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','qiandaodengji');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.qiantuijiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.qiantuijiluCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var qiandaoshijianChart1 = echarts.init(document.getElementById("qiandaoshijianChart1"),'macarons');
					let params = {}
					...
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							qiandaoshijianChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								qiandaoshijianChart1.resize();
							};
						}
					})
				})
