		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('qiantuijilu','新增')" type="primary" @click="add('/index/qiantuijiluAdd')">
						<span class="icon iconfont icon-tianjia14"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('qiantuijilu','用户签退统计')" type="success" @click="chartDialogShow1">
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`qiantuijilu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/qiantuijiluDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("qiantuijilu/group/qiantuishijian",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
