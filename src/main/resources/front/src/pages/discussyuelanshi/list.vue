		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('discussyuelanshi','新增')" type="primary" @click="add('/index/discussyuelanshiAdd')">
				...
				<el-table class="tables" :stripe='false'
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="table-del" type="success" @click.native="delClick(scope.row)" v-if="btnAuth('yuelanshi','删除评论')">
								<span class="icon iconfont icon-shanchu6"></span>
								删除
							</el-button>
							<el-button class="table-btn1" type="success" @click.native="discussClick(scope.row)" v-if="btnAuth('discussyuelanshi','查看评论')">
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`discussyuelanshi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/discussyuelanshiDetail', query: params});
			},
			...
			discussClick (row){
				...
				this.$router.push({path: '/index/discussdiscussyuelanshi', query: params});
			},
			editClick (row){
				this.$router.push(`/index/discussyuelanshiAdd?type=edit&&id=${row.id}`);
			},
			async delClick (row){
				await this.$confirm('是否删除此评论？') .then(_ => {
					this.$http.post('discussyuelanshi/delete', [row.id]).then(async res => {
						if (res.data.code == 0) {
							await this.$http.get('yuelanshi/info/' + row.refid).then(async rs=>{
								rs.data.data.discussnum--
								await this.$http.post('yuelanshi/update',rs.data.data).then(rs2=>{})
							})
							this.$message({
								message: "操作成功",
								type: "success",
			...
			discussistopChange(e,row){
				this.$http.post('discussyuelanshi/update',row).then(res=>{})
			},
		}
	}
</script>
