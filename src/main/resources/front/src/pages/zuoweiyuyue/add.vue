...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="座位状态" prop="reservationstate">
				<el-select v-model="ruleForm.reservationstate" placeholder="请选择座位状态" :disabled="ro.reservationstate"  filterable>
					<el-option
			...
			<el-form-item class="add-item"  label="签到状态" prop="qiandaozhuangtai">
				<el-select v-model="ruleForm.qiandaozhuangtai" placeholder="请选择签到状态" :disabled="ro.qiandaozhuangtai"  filterable>
					<el-option
			...
			<el-form-item class="add-item" label="学院" prop="xueyuan">
				<el-input v-model="ruleForm.xueyuan" 
					placeholder="学院" clearable :readonly="ro.xueyuan"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="班级" prop="banji">
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					this.ruleForm.reservationstate = '已预定'; 					this.ruleForm.qiandaozhuangtai = '待签到'; 				}else if(type=='edit'){
					this.info()
				}
				// 获取用户信息
				this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						...
						if((json.xueyuan!=''&&json.xueyuan) || json.xueyuan==0){
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`zuoweiyuyue/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`zuoweiyuyue/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
