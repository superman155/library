		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('zuoweiyuyue','新增')" type="primary" @click="add('/index/zuoweiyuyueAdd')">
						<span class="icon iconfont icon-tianjia14"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('zuoweiyuyue','预约统计')" type="success" @click="chartDialogShow1">
				...
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)" >
						...
						<div class="infoBox">
							...
							<div class="name">{{item.reservationstate}}</div>
							<div class="name">签到状态:{{item.qiandaozhuangtai}}</div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`zuoweiyuyue/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/zuoweiyuyueDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					this.$http.get("zuoweiyuyue/group/reservationdate",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
