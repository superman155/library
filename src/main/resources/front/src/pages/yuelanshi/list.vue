		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item" >
					...
					<el-select v-model="formSearch.zangshuleixing" placeholder="请选择藏书类型" :clearable="true">
						<el-option v-for="(item, index) in zangshuleixingOptions" :key="index" :label="item" :value="item"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item class="list-item" >
					<div class="lable">分区：</div>
					<el-select v-model="formSearch.fenqu" placeholder="请选择分区" :clearable="true">
						<el-option v-for="(item, index) in fenquOptions" :key="index" :label="item" :value="item"></el-option>
					</el-select>
				</el-form-item>
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('yuelanshi','新增')" type="primary" @click="add('/index/yuelanshiAdd')">
						<span class="icon iconfont icon-tianjia14"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('yuelanshi','阅览室浏览统计')" type="success" @click="chartDialogShow1">
...
<script>
	...
	export default {
		...
		async created() {
			...
			await this.$http.get('option/zangshuleixing/zangshuleixing').then(res => {
				if (res.data.code == 0) {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.mingcheng != ''&&this.formSearch.mingcheng != undefined) searchWhere.mingcheng = '%' + this.formSearch.mingcheng + '%';
				if (this.formSearch.zangshuleixing != ''&&this.formSearch.zangshuleixing != undefined) searchWhere.zangshuleixing = this.formSearch.zangshuleixing;
				if (this.formSearch.fenqu != ''&&this.formSearch.fenqu != undefined) searchWhere.fenqu = this.formSearch.fenqu;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`yuelanshi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/yuelanshiDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http.get("yuelanshi/value/mingcheng/clicknum",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
