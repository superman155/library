...
<template>
	<div class="main-content" :style='{"padding":"50px 30px 30px"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"border":"1px solid #00000050","padding":"0","margin":"0 0 0px","borderRadius":"10px 10px 0 0","background":"#fff","borderWidth":"1px 1px 0","width":"100%"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 10px 0px","boxShadow":"none","borderColor":"#00000050","borderRadius":"0px","flexWrap":"wrap","borderWidth":"0 0 1px","display":"flex","width":"100%","position":"relative","borderStyle":"solid"}' >
					<div  :style='{"margin":"0 1% 20px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">名称</label>
						<el-input v-model="searchForm.mingcheng" placeholder="名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"margin":"0 1% 20px 0","display":"flex"}' class="select" label="藏书类型" prop="zangshuleixing">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">藏书类型</label>
						<el-select clearable v-model="searchForm.zangshuleixing" placeholder="请选择藏书类型" >
							<el-option v-for="(item,index) in zangshuleixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div  :style='{"margin":"0 1% 20px 0","display":"flex"}' class="select" label="分区" prop="fenqu">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">分区</label>
						<el-select clearable v-model="searchForm.fenqu" placeholder="请选择分区" >
							<el-option v-for="(item,index) in fenquOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","padding":"10px","margin":"0px 0 0px -4px","flexWrap":"wrap","justifyContent":"flex-end","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yuelanshi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('yuelanshi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yuelanshi','阅览室浏览统计')" type="success" @click="chartDialogShow1">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						阅览室浏览统计
					</el-button>
				</el-row>
			</el-form>
			<div :style='{"border":"1px solid #00000050","width":"100%","padding":"0","boxShadow":"0px 3px 6px #8558ff80","borderRadius":"0 0 10px 10px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yuelanshi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<!-- [Ljava.lang.String;@15133ce7 -->
							<!-- [Ljava.lang.String;@27c27c4d -->
							<el-button class="btn8" v-if="isAuth('yuelanshi','跨表预状态-预约')" @click="zuoweiyuyueCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								跨表预状态-预约
							</el-button>
							<el-button class="edit" v-if=" isAuth('yuelanshi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="view" v-if="isAuth('yuelanshi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('yuelanshi','删除')" type="primary" @click="deleteHandler(scope.row.id)">
	...
	export default {
		...
		methods: {
			...
			zuoweiyuyueCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','yuelanshi');
			...
			getDataList() {
				...
				if(this.searchForm.zangshuleixing!='' && this.searchForm.zangshuleixing!=undefined){
					params['zangshuleixing'] = this.searchForm.zangshuleixing
				}
				if(this.searchForm.fenqu!='' && this.searchForm.fenqu!=undefined){
					params['fenqu'] = this.searchForm.fenqu
				}
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussyuelanshi',query:{refid:id}});
			},
			// 删除
...
<style lang="scss" scoped>
	...
	.center-form-pv .search {
		...
		outline: none;
		color: #fff;
		background: linear-gradient(126.43deg, rgba(221, 61, 249, 1),rgba(125, 78, 251, 1) 100%);
		width: auto;
		font-size: 16px;
		height: 36px;
	}
