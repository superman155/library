...
<template>
	<div class="main-content" :style='{"padding":"50px 30px 30px"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"border":"1px solid #00000050","padding":"0","margin":"0 0 0px","borderRadius":"10px 10px 0 0","background":"#fff","borderWidth":"1px 1px 0","width":"100%"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","padding":"10px","margin":"0px 0 0px -4px","flexWrap":"wrap","justifyContent":"flex-end","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('zuoweiyuyue','预约统计')" type="success" @click="chartDialogShow1">
				...
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<!-- [Ljava.lang.String;@3028026a -->
							<!-- [Ljava.lang.String;@32eae4fd -->
							<el-button class="btn8" v-if="isAuth('zuoweiyuyue','取消预约')" @click="quxiaoyuyueCrossAddOrUpdateHandler(scope.row,'cross','是','','reservationstate','已取消','已取消,已预约'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								取消预约
							</el-button>
							<el-button class="btn8" v-if="isAuth('zuoweiyuyue','签到')" @click="qiandaodengjiCrossAddOrUpdateHandler(scope.row,'cross','否','','qiandaozhuangtai','已签到','已签到,待签到'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								签到
							</el-button>
		...
		<qiandaodengji-cross-add-or-update v-if="qiandaodengjiCrossAddOrUpdateFlag" :parent="this" ref="qiandaodengjiCrossaddOrUpdate"></qiandaodengji-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="reservationdateChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
...
<script>
	...
	import qiandaodengjiCrossAddOrUpdate from "../qiandaodengji/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				qiandaodengjiCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			qiandaodengjiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "warning",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.qiandaodengjiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','zuoweiyuyue');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.qiandaodengjiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.qiandaodengjiCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var reservationdateChart1 = echarts.init(document.getElementById("reservationdateChart1"),'macarons');
					let params = {}
					...
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							reservationdateChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								reservationdateChart1.resize();
							};
						}
					})
				})
