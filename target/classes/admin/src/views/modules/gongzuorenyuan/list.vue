		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"border":"1px solid #00000050","padding":"0","margin":"0 0 0px","borderRadius":"10px 10px 0 0","background":"#fff","borderWidth":"1px 1px 0","width":"100%"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","padding":"10px","margin":"0px 0 0px -4px","flexWrap":"wrap","justifyContent":"flex-end","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('gongzuorenyuan','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
				...
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<!-- $crossOptTableName -->
							<!-- $crossOptButton -->
							<el-button class="edit" v-if=" isAuth('gongzuorenyuan','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "gongzuorenyuan/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
