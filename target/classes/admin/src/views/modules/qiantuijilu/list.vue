...
<template>
	<div class="main-content" :style='{"padding":"50px 30px 30px"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"border":"1px solid #00000050","padding":"0","margin":"0 0 0px","borderRadius":"10px 10px 0 0","background":"#fff","borderWidth":"1px 1px 0","width":"100%"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","padding":"10px","margin":"0px 0 0px -4px","flexWrap":"wrap","justifyContent":"flex-end","display":"flex"}'>
					<el-button class="add" v-if="isAuth('qiantuijilu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('qiantuijilu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#fff","height":"auto"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('qiantuijilu','用户签退统计')" type="success" @click="chartDialogShow1">
			...
			<div :style='{"border":"1px solid #00000050","width":"100%","padding":"0","boxShadow":"0px 3px 6px #8558ff80","borderRadius":"0 0 10px 10px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('qiantuijilu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<!-- $crossOptTableName -->
							<!-- $crossOptButton -->
							<el-button class="edit" v-if=" isAuth('qiantuijilu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('qiantuijilu','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<el-dialog
			...
			width="800">
			<div id="qiantuishijianChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var qiantuishijianChart1 = echarts.init(document.getElementById("qiantuishijianChart1"),'macarons');
					let params = {}
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							qiantuishijianChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								qiantuishijianChart1.resize();
							};
						}
					})
				})
