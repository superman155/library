		...
		<el-form
			...
			<el-form-item class="add-item" label="论坛id" prop="forumid">
				<el-input v-model="ruleForm.forumid" 
					placeholder="论坛id" clearable :readonly="ro.forumid"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item"  label="状态" prop="status">
				<el-select v-model="ruleForm.status" placeholder="请选择状态" :disabled="ro.status"  filterable>
					<el-option
			...
			<el-form-item class="add-item"  label="举报类型" prop="reporttype">
				<el-select v-model="ruleForm.reporttype" placeholder="请选择举报类型" :disabled="ro.reporttype"  filterable>
					<el-option
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`forumreport/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
