		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item" >
					...
					<el-select v-model="formSearch.xuexifengge" placeholder="请选择学习风格" :clearable="true">
						<el-option v-for="(item, index) in xuexifenggeOptions" :key="index" :label="item" :value="item"></el-option>
					</el-select>
				</el-form-item>
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('xuexidazi','新增')" type="primary" @click="add('/index/xuexidaziAdd')">
				...
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)" >
						...
						<div class="infoBox">
							...
							<div class="name">{{item.xuexifengge}}</div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.biaoti != ''&&this.formSearch.biaoti != undefined) searchWhere.biaoti = '%' + this.formSearch.biaoti + '%';
				if (this.formSearch.xuexifengge != ''&&this.formSearch.xuexifengge != undefined) searchWhere.xuexifengge = this.formSearch.xuexifengge;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`xuexidazi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/xuexidaziDetail', query: params});
			},
