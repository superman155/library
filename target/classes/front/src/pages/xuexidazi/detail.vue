	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/xuexidazi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">学习风格</div>
						<div class="text "  >{{detail.xuexifengge}}</div>
					</div>
					<div class="item">
						<div class="lable">监督机制</div>
						<div class="text "  >{{detail.jiandujizhi}}</div>
					</div>
					<div class="item">
						<div class="lable">搭子类型</div>
						<div class="text "  >{{detail.dazileixing}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('xuexidazi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('xuexidazi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('xuexidazi','申请')" 
							@click="onAcross('shenqingjiaru','','','[1]','已申请')" type="warning">
							申请
						</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('xuexidazi','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`xuexidazi`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/xuexidazi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/xuexidaziAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此学习搭子？') .then(_ => {
					this.$http.post('xuexidazi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapterxuexidazi', query: params});
			},
			// 获取uuid
			getUUID() {
