...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="学习风格" prop="xuexifengge">
				<el-input v-model="ruleForm.xuexifengge" 
					placeholder="学习风格" clearable :readonly="ro.xuexifengge"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="监督机制" prop="jiandujizhi">
				<el-input v-model="ruleForm.jiandujizhi" 
					placeholder="监督机制" clearable :readonly="ro.jiandujizhi"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="搭子类型" prop="dazileixing">
				<el-input v-model="ruleForm.dazileixing" 
					placeholder="搭子类型" clearable :readonly="ro.dazileixing"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="申请时间" prop="shenqingshijian">
				<el-date-picker
					...
					type="date"
					:picker-options="shenqingshijianPickerOptions"
					placeholder="申请时间">
				</el-date-picker> 
			</el-form-item>
			...
			<el-form-item class="add-item" label="加入原因" prop="jiaruyuanyin">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.jiaruyuanyin"
					placeholder="加入原因"
					v-model="ruleForm.jiaruyuanyin">
					</el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='jiaruyuanyin'){
							this.ruleForm.jiaruyuanyin = obj[o];
							this.ro.jiaruyuanyin = true;
							continue;
						}
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`shenqingjiaru/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
					...
					if(valid) {
						...
						if(this.type=='cross'){
							...
							if(crossrefid && crossuserid) {
								...
								await this.$http.get('shenqingjiaru/list', {
									params: params
								}).then(res => {
						...
						await this.$http.post(`shenqingjiaru/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
