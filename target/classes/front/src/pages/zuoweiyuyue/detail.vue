	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/zuoweiyuyue?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">座位号</div>
						<div class="text "  >{{detail.seatnum}}</div>
					</div>
					<div class="item">
						<div class="lable">预约日期</div>
						<div class="text "  >{{detail.reservationdate}}</div>
					</div>
					<div class="item">
						<div class="lable">时间段</div>
						<div class="text "  >{{detail.timeslot}}</div>
					</div>
					<div class="item">
						<div class="lable">座位状态</div>
						<div class="text "  >{{detail.reservationstate}}</div>
					</div>
					<div class="item">
						<div class="lable">签到状态</div>
						<div class="text "  >{{detail.qiandaozhuangtai}}</div>
					</div>
					<div class="item">
						<div class="lable">预约备注</div>
						<div class="text "  >{{detail.yuyuebeizhu}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('zuoweiyuyue','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('zuoweiyuyue','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('zuoweiyuyue','取消预约')" 
							@click="onAcross('quxiaoyuyue','是','','reservationstate','已取消','已取消,已预约'.split(',')[0])" type="warning">
							取消预约
						</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('zuoweiyuyue','签到')" 
							@click="onAcross('qiandaodengji','否','','qiandaozhuangtai','已签到','已签到,待签到'.split(',')[0])" type="warning">
							签到
						</el-button>
						<el-button class="auditBtn" v-if="btnAuth('zuoweiyuyue','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('zuoweiyuyue','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				if (this.detail.reservationstate=='已取消'){
					this.$message.error('该预约已取消，不能再操作');
					return;
				}
				...
				localStorage.setItem('crossTable',`zuoweiyuyue`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/zuoweiyuyue', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/zuoweiyuyueAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此座位预约？') .then(_ => {
					this.$http.post('zuoweiyuyue/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapterzuoweiyuyue', query: params});
			},
			// 获取uuid
			getUUID() {
